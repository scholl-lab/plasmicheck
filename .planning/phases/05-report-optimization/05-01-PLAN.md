...
must_haves:
  truths:
    ...
    - "run_pipeline passes static_report, plotly_mode, and output_root to generate_report"
  artifacts:
    ...
    - path: "plasmicheck/scripts/run_pipeline.py"
      provides: "Flag pass-through from pipeline to generate_report including output_root"
      contains: "output_root"
  key_links:
    ...
    - from: "plasmicheck/scripts/run_pipeline.py"
      ...
      via: "generate_report() call with static_report, plotly_mode, and output_root kwargs"
      pattern: "generate_report.*output_root"
---
...
Output: Updated cli.py with new flags on three subcommands, updated run_pipeline.py that accepts and forwards the new parameters (including output_root for shared assets directory).
</objective>
...
4. Update the `report` command handler to pass new flags to `generate_report()`. NOTE: In cli.py, `generate_report` is already the `main()` function (imported as `from .scripts.generate_report import main as generate_report`), so calling `generate_report()` IS calling `main()`. Add the new kwargs:
   ```python
   generate_report(
       ...existing args...,
       static_report=args.static_report,
...
5. Update the `summary_reports` command handler to pass new flags to `generate_summary_reports()`. NOTE: Same pattern — `generate_summary_reports` is imported as `from .scripts.generate_summary_reports import main as generate_summary_reports`, so it IS calling `main()`. Add the new kwargs:
   ```python
...
<task type="auto">
  <name>Task 2: Wire flags through run_pipeline to generate_report (including output_root)</name>
  <files>plasmicheck/scripts/run_pipeline.py</files>
  <action>
Update run_pipeline.py to accept and forward the new report flags, including the output_root parameter needed for Plan 05-02's shared assets/ directory:

1. Add `static_report`, `plotly_mode`, and note about output_root to `run_pipeline()` function signature:
   ```python
   def run_pipeline(
       ...existing params...,
...
2. In the pipeline execution section (Step 7: Generate report, around line 575), pass the new flags AND `output_root` to the `generate_report()` call. The `output_folder` parameter of `run_pipeline()` is the top-level output directory (e.g., `output/`), while `output_subfolder` is the per-sample-plasmid directory (e.g., `output/sample/plasmid/`). For directory-mode plotly.js, the shared `assets/` directory belongs at the root level. Pass `output_root=output_folder`:
   ```python
   generate_report(
   ...
   This is critical: `output_folder` (run_pipeline's parameter) is the root that Plan 05-02 will use for the shared `output/assets/plotly.min.js` directory. Without passing it, generate_report would have to guess the root by walking up from `output_subfolder`, which is fragile.
...
NOTE: After this task, `run_pipeline()` accepts the new flags and passes them through (including output_root). However, `generate_report.main()` doesn't accept them yet, so a full pipeline run would fail. This is expected — Plans 02 and 03 complete the wiring.
  </action>
  <verify>
Run `make lint` — no new ruff errors.
Run `make typecheck` — check for mypy errors. There may be expected errors about generate_report.main() not accepting static_report/plotly_mode/output_root kwargs. Note these but they're expected.
Run `python -c "from plasmicheck.scripts.run_pipeline import run_pipeline; import inspect; sig = inspect.signature(run_pipeline); print(sig)"` to confirm new parameters are in the signature.
  ...
  <done>
run_pipeline() function signature includes static_report: bool = False and plotly_mode: str = "directory". These values plus output_root=output_folder are forwarded to the generate_report() call inside the pipeline loop. The __main__ block also includes the new flags.
  </done>
</task>
...
6. generate_report() call in run_pipeline includes output_root=output_folder
</verification>
...
- run_pipeline() forwards both flags plus output_root to generate_report() call
